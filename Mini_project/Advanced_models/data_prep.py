...
import scandir
from transformers import ElectraTokenizer,XLNetTokenizer, BertTokenizer, RobertaTokenizer, DistilBertTokenizer, AutoTokenizer
...
def get_data(base_dir, arch):
    ...
    allowed_arch = ['electra', 'bert', 'roberta', 'distilbert','sent_roberta', 'xlnet']
    if arch not in allowed_arch:
        raise Exception('Invalid architecture, only allowed: electra, bert, roberta, xlnet')
    neg = base_dir + '/neg'
    pos = base_dir + '/pos'
...
def get_train(arch):
    base_dir='Data/aclImdb_v1/aclImdb/train'
    return get_data(base_dir, arch)

def get_test(arch):
    base_dir='Data/aclImdb_v1/aclImdb/test'
    return get_data(base_dir, arch)
